export function printOperatorData(operatorList, pageNumber, pageSize) {
    let start = (pageNumber + 1) * pageSize - pageSize + 1;
    operatorList.forEach((element) => {
        $("#operatorData").append("<tr><th scope='col'>" +
            start +
            "</th><td>" +
            element.companyName +
            "</td><td>" +
...
export function printFlightData(flightList, pageNumber, pageSize) {
    let start = (pageNumber + 1) * pageSize - pageSize + 1;
    flightList.forEach((element) => {
        $("#flightData").append("<tr><th scope='col'>" +
            start +
            "</th><td>" +
...
export function printAirportData(airportList, pageNumber, pageSize) {
    let start = (pageNumber + 1) * pageSize - pageSize + 1;
    airportList.forEach((element) => {
        $("#airportData").append("<tr><th scope='col'>" +
            start +
            "</th><td>" +
            element.name +
            "</td><td>" +
