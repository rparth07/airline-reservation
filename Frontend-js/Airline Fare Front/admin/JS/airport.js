...
function filterData(event, paginateAirport) {
    event.preventDefault();
    let filteredAirports = [];
    let inputElement, filter;
    ...
    filteredAirports = elementData.filter(function (airport) {
        return (airport.name.toUpperCase().indexOf(filter) > -1 ||
...
function bindAirportSuggestionEvent(paginateAirport) {
    ...
    $("input").on("click", function () {
        ...
        $(searchInput).on("keyup", function () {
            ...
            renderCityChoice(searchInput, suggestionListElement, suggestionString, paginateAirport);
        });
    });
}
function renderCityChoice(searchInput, suggestionListElement, suggestionString, paginateAirport) {
...
function bindPageChangeEvent(paginateAirport) {
    $(".pageChange").on("click", function () {
        changePageAndPrintData(paginateAirport, parseInt(this.getAttribute("data-page-number"), 10));
    });
}
function changePageAndPrintData(paginateAirport, currentPage) {
    paginateAirport.pageChange(currentPage, "#airportData");
    printAirportData(paginateAirport.getCurrentPageData(), currentPage, paginateAirport.getPageSize());
}
