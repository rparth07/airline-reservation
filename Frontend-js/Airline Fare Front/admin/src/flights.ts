...
function filterData(event: JQuery.Event, paginateFlight: Paginate<Flight>) {
  ...
  filteredFlights = elementData.filter(function (flight) {
    return (
      flight.sourceCity.toUpperCase().indexOf(filter) > -1 ||
...
function bindFlightSuggestionEvent(paginateFlight: Paginate<Flight>) {
  ...
  $("input").on("click", function () {
    ...
    $(searchInput).on("keyup", function () {
      ...
      for (var flight of flights) {
        if (
          flight.sourceCity.toLowerCase().startsWith(input.toLowerCase()) &&
...
function renderCityChoice(
  ...
  paginateFlight: Paginate<Flight>
) {
...
function bindPageChangeEvent(paginateFlight: Paginate<Flight>) {
...
function changePageAndPrintData(
  ...
  printFlightData(
    paginateFlight.getCurrentPageData(),
    currentPage,
    paginateFlight.getPageSize()
  );
}
