...
enum DateType {
  departurDateSelector = ".departureDate",
  arrivalDateSelector = ".arrivalDate",
}
...
function dateSelectorOnChangeEventListener() {
  $(DateType.departurDateSelector).on("change", function () {
    let thisElement = this as HTMLInputElement;
...
export function invertTripCity() {
  $(".reverse-icon").on("click", function () {
    const sourceSearchInput: string = ".source-search-input";
    const destinationSearchInput: string = ".destination-search-input";
...
export function bindCloseSuggestionEvent() {
  $(document).on("mouseup", function (e: JQuery.MouseUpEvent) {
    let container = $(".travellers-card");
    let sourceSuggestionContainer = $(".source-suggestions");
    let destinationSuggestionContainer = $(".destination-suggestions");
...
function removeListForEmptySuggestions(
  input: string,
