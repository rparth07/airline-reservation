...
$(function () {
  ...
  $(addCityButton).on("click", function () {
    let parentToAppendBefore = $(this).parent();
    ...
    let addedCityRow = $(parentToAppendBefore).prev(
      "div"
...
function removeMultipleCityListner(
  citySelection: JQuery<HTMLDivElement>,
  addCityButton: JQuery<HTMLElement>
) {
  let removeButton = $(citySelection).find(".remove-city-button"); //".remove-city-button"
...
function handlefromCityButtons(addCityButton: JQuery<HTMLElement>) {
  if (noOfCity == 5) {
